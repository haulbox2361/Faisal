...
//
// SINGLE-INSTANCE ONLY: This Map is process-local. If HaulBoX ever runs behind a
// load balancer with multiple Node.js instances, Instance A's cache will not be
// invalidated when Instance B writes a new value — it will serve stale data for
// up to CACHE_TTL_MS milliseconds. Migration path: replace `kvCache` (Map) with a
// shared Redis client using the same get/set/delete interface. CACHE_TTL_MS controls
// the Redis TTL via SETEX. Current deployment is single-dyno, so this is zero risk.
const kvCache = new Map();
const CACHE_TTL_MS = 15000; // 15 seconds — balances freshness with DB read reduction
