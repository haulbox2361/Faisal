...
function evaluateBolVerification(ai, load, ocrError, options = {}) {
  ...
  if (!isDoc || (detectedType !== 'BOL' && detectedType !== 'BILL OF LADING' && detectedType !== 'POD') || typeConfidence < 0.55) {
    ...
    return {
      ...
      validationResults,
      reason: `Uploaded photo is not a Bill of Lading (detected: ${detectedType || 'non-document'}). Please upload a clear photo of the printed BOL paper.`,
  ...
  // NOTE: OCR is NEVER allowed to set status to APPROVED.
  // The APPROVED state can ONLY be set by a human Dispatcher/Admin/Super Admin
  // via the /api/documents/review or /api/documents/review-action endpoint.
  // OCR produces either PENDING_REVIEW (manual review required) or REJECTED
  // (image/document quality too poor to review).
...
function evaluatePodVerification(ai, load, ocrError, options = {}) {
  ...
  // NOTE: OCR is NEVER allowed to set status to APPROVED.
  // APPROVED can ONLY be set by a human Dispatcher/Admin/Super Admin reviewer.
  const finalStatus = 'PENDING_REVIEW';
  const reviewReason = issues.length > 0 ? issues.join('; ') : 'Awaiting Dispatcher/Admin review';
