...
// Retrieve Mistral AI key from env or app_settings KV
...
async function callMistralVisionVerification({ apiKey, base64, mimeType, expectedType, expectedLoad }) {
  const prompt = `You are a strict, highly accurate production logistics document verification AI.
Inspect the uploaded image carefully to determine if it is a valid ${expectedType} (Bill of Lading or Proof of Delivery).

Expected Load Specs:
- Load #: ${expectedLoad.loadNumber || expectedLoad.id || 'HB-1042'}
- Expected Weight: ${expectedLoad.weight || '42500'} lbs
- Expected Pickup: ${expectedLoad.pickupAddress || expectedLoad.pickup || ''}
- Expected Delivery: ${expectedLoad.dropoffAddress || expectedLoad.dropoff || ''}

STRICT INSPECTION RULES:
1. Document Identification: Check if the photo is a real freight document. If it is a photo of random objects, a person, scenery, a dark screen, receipt, or non-freight paper, set "isDocument": false, "detectedType": "UNKNOWN", and "confidence": 0.2.
2. Signatures:
   - For BOL: inspect the shipper signature line at the bottom. If it is empty or unsigned, set "shipperSignatureDetected": false and "signatureDetected": false.
   - For POD: inspect the consignee/receiver signature line. If empty or unsigned, set "consigneeSignatureDetected": false.
3. Extraction: Extract the actual text visible on the document. Do not invent or assume data. If not visible, return null or empty string.
4. Image Quality: Set "isClear": false if blurred; set "cornersVisible": false if text or edges are cut off.
