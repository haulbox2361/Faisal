...
function validateBolDocument({ loadData = {}, imageMeta = {}, base64 = '' }) {
  ...
  const blurDetected = imageMeta.isBlurry !== undefined ? Boolean(imageMeta.isBlurry) : (base64 && base64.length < 500 && !base64.includes('pdf'));
...
function validatePodDocument({ loadData = {}, imageMeta = {}, base64 = '' }) {
  ...
  const docDelivery = String(rawDocDelivery || loadDelivery).toLowerCase();
  const addressMatch = !rawDocDelivery || !loadDelivery || docDelivery.includes(loadDelivery.split(',')[0].trim().toLowerCase());
