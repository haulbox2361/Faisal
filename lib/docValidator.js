...
function validateBolDocument({ loadData = {}, imageMeta = {}, base64 = '' }) {
  ...
  if (blurDetected || !cornersVisible || !signaturePresent) {
    ...
    // Clean document with all metadata checks passing — held for human review (OCR cannot auto-approve)
  ...
  return {
    documentType: 'BOL',
    overallStatus,
    confidence: issues.length === 0 ? 0.96 : 0.65,
    clarityPass,
    blurDetected,
    shadowDetected,
    cornersVisible,
...
function validatePodDocument({ loadData = {}, imageMeta = {}, base64 = '' }) {
  ...
  return {
    documentType: 'POD',
    overallStatus,
    confidence: issues.length === 0 ? 0.98 : 0.70,
    clarityPass,
    blurDetected,
    shadowDetected,
    cornersVisible,
