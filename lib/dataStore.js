...
async function loadFullState() {
  const layers = await getReadLayers();
  const allRelational = Object.values(layers).every(v => v === 'relational');
  ...
  const kvState = (await loadStateFromKv()) || { dispatchers: [], brokers: [], drivers: [], loads: [], settings: {} };
