...
async function loadFullState() {
  ...
  try {
    ...
    if (layers.loads === 'relational') {
      const query = `
        SELECT id, load_number, broker_id, broker_name, broker_phone, driver_id, dispatcher_id,
               company_id, status, pickup_city, pickup_state, pickup_date, delivery_city,
               delivery_state, delivery_date, miles, rate, driver_pay, weight, commodity,
               trailer_type, equipment_notes, tracking_status, checkpoints, documents,
