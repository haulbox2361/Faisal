...
class MockPool {
  async query(sql, params = []) {
    ...
    if (s.startsWith('CREATE') || s.startsWith('DROP') || s.startsWith('SELECT 1 FROM information_schema')) {
      return { rows: [] };
    }
    ...
    if (s.includes('INSERT INTO driver_sessions')) {
      memStore.sessions.set(params[0], { driver_id: params[1], expires_at: Date.now() + 30*86400000 });
      return { rows: [] };
    }
    if (s.includes('UPDATE driver_sessions')) {
      const sess = memStore.sessions.get(params[0]);
      return { rows: sess ? [{ driver_id: sess.driver_id }] : [] };
    }
