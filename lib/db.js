...
class MockPool {
  async query(sql, params = []) {
    ...
    if (s.includes('SELECT value FROM kv_store WHERE key=$1') || (s.startsWith('SELECT') && s.includes('FROM kv_store WHERE key=$1'))) {
    ...
    if (s.includes('DELETE FROM driver_sessions')) {
      if (s.includes('driver_id =') && params.length > 0) {
        for (const [k, v] of memStore.sessions.entries()) {
          if (v && v.driver_id === params[0]) memStore.sessions.delete(k);
        }
      } else {
    ...
    if (s.includes('DELETE FROM companies')) {
      memStore.companies = (memStore.companies || []).filter(c => String(c.id) !== String(params[0]));
      return { rows: [{ id: params[0] }] };
    }
    if (s.includes('DELETE FROM drivers')) {
      memStore.drivers = (memStore.drivers || []).filter(d => String(d.id) !== String(params[0]));
      return { rows: [{ id: params[0] }] };
    }
    if (s.includes('DELETE FROM owners')) {
      if (s.includes('company_id =')) {
        memStore.owners = (memStore.owners || []).filter(o => String(o.company_id) !== String(params[0]));
      } else {
        memStore.owners = (memStore.owners || []).filter(o => String(o.id) !== String(params[0]));
      }
      return { rows: [] };
    }
    if (s.includes('DELETE FROM driver_locations')) {
      return { rows: [] };
    }
    ...
    if (s.includes('INSERT INTO notifications')) {
      memStore.notifications = memStore.notifications || [];
      const entry = {
        id: memStore.notifications.length + 1,
        recipient_type: params[0],
        recipient_id: params[1],
        type: params[2],
        title: params[3],
        body: params[4] || null,
        data: params[5] ? (typeof params[5] === 'string' ? JSON.parse(params[5]) : params[5]) : null,
        read_at: null,
        created_at: new Date().toISOString()
      };
      memStore.notifications.push(entry);
      return { rows: [entry] };
    }
    if (s.includes('FROM notifications')) {
      memStore.notifications = memStore.notifications || [];
      let list = memStore.notifications.slice();
      if (params && params.length >= 2) {
        list = list.filter(n => n.recipient_type === params[0] && String(n.recipient_id) === String(params[1]));
      }
      if (s.includes('read_at IS NULL')) {
    ...
    if (s.includes('UPDATE notifications')) {
      memStore.notifications = memStore.notifications || [];
      let rowCount = 0;
      if (s.includes('read_at = now()') && params && params.length >= 3) {
        const notifId = Number(params[0]);
        const rType = params[1];
        const rId = String(params[2]);
        const found = memStore.notifications.find(n => n.id === notifId && n.recipient_type === rType && String(n.recipient_id) === rId);
...
async function ensureSchema() {
  ...
  schemaReady = pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS load_history_timeline_idx ON load_history(load_id, created_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS notifications_unread_idx ON notifications(recipient_type, recipient_id, read_at);
...
async function deleteCompany(id) {
  await ensureSchema();
  const pool = getPool();
  // 1. Reassign drivers and loads to default legacy company
  await pool.query(`UPDATE drivers SET company_id = 'COMP-LEGACY' WHERE company_id = $1`, [id]).catch(() => {});
  await pool.query(`UPDATE loads SET company_id = 'COMP-LEGACY' WHERE company_id = $1`, [id]).catch(() => {});
  // 2. Delete linked owners
  await pool.query(`DELETE FROM owners WHERE company_id = $1`, [id]).catch(() => {});
...
async function deleteDriver(id) {
  await ensureSchema();
  const pool = getPool();
  // 1. Clean up driver sessions, locations, and unassign from loads
  await pool.query(`DELETE FROM driver_sessions WHERE driver_id = $1`, [id]).catch(() => {});
  await pool.query(`DELETE FROM driver_locations WHERE driver_id = $1`, [id]).catch(() => {});
  await pool.query(`UPDATE loads SET driver_id = NULL WHERE driver_id = $1`, [id]).catch(() => {});
  // 2. Delete driver record
  const res = await pool.query(`DELETE FROM drivers WHERE id = $1 RETURNING *`, [id]);
  return res.rows[0] || null;
}
