...
async function saveOwner({ id, ownerCode, pinHash, name, phone, email, active = true }) {
  await ensureSchema();
  const pool = getPool();
  const ownerId = id || ('OWN-' + Date.now().toString(36).toUpperCase());
  const res = await pool.query(
    `INSERT INTO owners (id, owner_code, pin_hash, name, phone, email, active, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
     ON CONFLICT (id) DO UPDATE SET
       ...
       updated_at = NOW()
     RETURNING *`,
    [ownerId, String(ownerCode).trim().toUpperCase(), pinHash, name, phone || '', email || '', active !== false]
  );
  return res.rows[0];
}
...
async function getAllOwners() {
  await ensureSchema();
  const pool = getPool();
  const res = await pool.query(`SELECT id, owner_code, name, phone, email, active, created_at, updated_at FROM owners ORDER BY name ASC`);
  return res.rows;
}
