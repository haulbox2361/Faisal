...
async function create(recipientType, recipientId, { type, title, body, data }) {
  ...
  return {
    id: rows[0] ? rows[0].id : 'notif-' + Date.now(),
    recipientType,
    recipientId,
    type,
    title,
    ...
    data: data || null,
    read: false,
    createdAt: rows[0] ? rows[0].created_at : new Date().toISOString()
  };
}
...
async function clearAll(recipientType, recipientId) {
  await ensureSchema();
  const { rowCount } = await getPool().query(
...
module.exports = { create, listFor, unreadCount, markRead, markAllRead, clearAll };
